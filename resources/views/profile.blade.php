...
let cropper;
let cropSize = 300;
const sizeInput = document.getElementById('cropper-size');
const sizeValue = document.getElementById('cropper-size-value');
sizeInput.addEventListener('input', function() {
    cropSize = parseInt(this.value);
...
document.getElementById('avatar').addEventListener('change', function(e) {
    ...
    reader.onload = function(event) {
        document.getElementById('cropper-preview').src = event.target.result;
        document.getElementById('avatar-cropper-modal').classList.remove('hidden');
        if (cropper) cropper.destroy();
        cropper = new Cropper(document.getElementById('cropper-preview'), {
            aspectRatio: 1,
            viewMode: 1,
        });
    };
...
document.getElementById('cropper-save').onclick = function() {
    ...
    document.getElementById('avatar-cropped').value = canvas.toDataURL('image/png');
    document.getElementById('avatar-cropper-modal').classList.add('hidden');
    cropper.destroy();
    cropper = null;
};
